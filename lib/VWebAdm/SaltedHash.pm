...
############################################
# Usage      : my $hash = VBoxAdm::SaltedHash::plain_md5('pwclear');
...
############################################
# Usage      : my $hash = VBoxAdm::SaltedHash::cram_md5('pwclear');
...
############################################
# Usage      : my $hash = VBoxAdm::SaltedHash::smd5('pwclear','salt');
...
############################################
# Usage      : my $hash = VBoxAdm::SaltedHash::sha('pwclear');
...
############################################
# Usage      : my $hash = VBoxAdm::SaltedHash::ssha('pwclear','salt');
...
############################################
# Usage      : my $hash = VBoxAdm::SaltedHash::sha256('pwclear');
...
############################################
# Usage      : my $hash = VBoxAdm::SaltedHash::ssha256('pwclear','salt');
...
############################################
# Usage      : my $hash = VBoxAdm::SaltedHash::sha512('pwclear');
...
############################################
# Usage      : my $hash = VBoxAdm::SaltedHash::ssha512('pwclear','salt');
...
############################################
# Usage      : my $hash = VBoxAdm::SaltedHash::make_pass('pwclear','pwscheme','salt');
...
############################################
# Usage      : my $hash = VBoxAdm::SaltedHash::make_salt();
...
# See Also   : http://wiki.dovecot.org/Authentication/PasswordSchemes
sub make_salt {
    return Data::Pwgen::pwgen(4);
}

############################################
# Usage      : my $hash = VBoxAdm::SaltedHash::pad_base64('pwclear');
...
############################################
# Usage      : my $hash = VBoxAdm::SaltedHash::verify_pass('pwclear','pwentry');
# Purpose    : ????
# Returns    : ????
# Parameters : ????
# Throws     : no exceptions
...
VBoxAdm::SaltedHash - Generate hashed passwords for use with Dovecot SASL.
...
This method turns a password into a hash which Dovecot can use for CRAM-MD5
as an auth string. This string consists of two 16-byte state arrays
which are generated by feeding two differntly padded version of the same
password into the MD5 hash algorithm and recording the (unfinalized) state arrays.

=head2 ldap_md5

A base64 encoded md5 hash of the input string.

=head2 make_pass

Process the given password with the requested schema.
...
Make sure the input string contains valid base64 padding, adding '=' chars if the length is not
disivisble be 4 w/o remainder.

=head2 plain_md5

Return the MD5 hash in hex encoding.
...
Return the base64 encoded SHA-512 hash.

=head2 smd5

Return the base64 encoded MD5 hash of input+random_salt

=head2 split_pass

Extract hashing schema, possible salt and password hash from input.

=head2 ssha

Return the base64 encoded salted SHA-1 hash.

=head2 ssha256

Return the base64 encoded salted SHA-256 hash.

=head2 ssha512

Return the base64 encoded salted SHA-512 hash.

=head2 verify_pass

Compare a given plaintext password against the hash.
