...
sub run {
    ...
    while ( my ( $domain, $is_active ) = $sth_domains->fetchrow_array() ) {
        ...
        # create a new domain
        $self->sth_new_domain()->execute( $domain, $is_active )
          or die( "Could not execute query: " . $self->sth_new_domain()->errstr );
        ...
        while ( my @row = $sth_mailboxes->fetchrow_array() ) {
            ...
            $self->sth_new_mailbox()->execute( $new_domain_id, $local_part, VWebAdm::SaltedHash::make_pass( $password, $self->config()->{'default'}{'pwscheme'} ),
                $name, $is_active, $maxmsgsize, $is_on_vacation, $vacation_subj, $vacation_msg, $is_domainadmin, $is_siteadmin, $sa_active, $sa_kill_score,
            ) or die( "Could not execute query: " . $self->sth_new_mailbox()->errstr );
            print
"New Mailbox: $new_domain_id, $local_part, $password, $name, $is_active, $maxmsgsize, $is_on_vacation,'$vacation_msg',$quota,$is_domainadmin,$is_siteadmin\n";
        ...
        while ( my ( $domain_alias, $is_active ) = $sth_alias_domains->fetchrow_array() ) {
            ...
            $self->sth_new_domain_alias()->execute( $domain_alias, $new_domain_id )
              or die( "Could not execute query: " . $self->sth_new_domain_alias()->errstr );
        }
    }
    ...
    while ( my ( $on_vacation, $notified, $notified_at ) = $sth_vacation_notify->fetchrow_array() ) {
        ...
        $self->sth_vacation_status()->execute( $on_vacation, $notified, $notified_at )
          or die( "Could not execute query: " . $self->sth_vacation_status()->errstr );
    }
    ...
    while ( my ( $email, $domain ) = $sth_domain_admin->fetchrow_array() ) {
        ...
        $self->sth_set_admin()->execute( 1, 0, $local_part, $domain )
          or die( "Could not execute query: " . $self->sth_set_admin()->errstr );
