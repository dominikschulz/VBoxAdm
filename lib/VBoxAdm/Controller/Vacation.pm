...
sub _init_logger {
    ...
    return Log::Tree::->new( { facility => 'vboxadm-vacation', filename => '/var/log/vboxadm/vacation.log', } );
}
...
sub _is_on_vacation {
    ...
    $query .= q{ AND (ma.vacation_start = '0000-00-00' OR DATE(NOW()) >= DATE(ma.vacation_start)) AND (ma.vacation_end = '0000-00-00' OR DATE(ma.vacation_end) >= DATE(NOW()))};
...
VBoxAdm::Controller::Vacation - Handling vacation replies
